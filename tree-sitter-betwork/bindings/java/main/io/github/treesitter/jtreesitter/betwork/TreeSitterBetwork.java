package io.github.treesitter.jtreesitter.betwork;

import java.lang.foreign.*;

public final class TreeSitterBetwork {
    ...
    private static final TreeSitterBetwork INSTANCE = new TreeSitterBetwork();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterBetwork() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_betwork");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-betwork");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-betwork");
