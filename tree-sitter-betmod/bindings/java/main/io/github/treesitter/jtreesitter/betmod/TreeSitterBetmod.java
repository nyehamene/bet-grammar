package io.github.treesitter.jtreesitter.betmod;

import java.lang.foreign.*;

public final class TreeSitterBetmod {
    ...
    private static final TreeSitterBetmod INSTANCE = new TreeSitterBetmod();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterBetmod() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_betmod");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-betmod");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-betmod");
